...
class pbmc_Nemo_Datamodule(DataInterfaceBase):
    ...
    def train_dataloader(self):
        ds = PBMC_Dataset(self.dataset_conf, self.train_env, self.control_env, self.global_keys, mode="train")
    ...
    def val_dataloader(self):
        ds = PBMC_Dataset(self.dataset_conf, self.val_env, self.control_env, self.global_keys, mode="val")
        prefetch_factor = 2 if int(self.num_workers) > 0 else None
        return DataLoader(
            ds,
            num_workers=self.num_workers,
    ...
    def test_dataloader(self):
        ds = PBMC_Dataset(self.dataset_conf, self.test_env, self.control_env, self.global_keys, mode="test")
        prefetch_factor = 2 if int(self.num_workers) > 0 else None
        return DataLoader(
            ds,
            num_workers=self.num_workers,
