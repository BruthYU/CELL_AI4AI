...
class PBMC_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample = {
            "batch_onehot": self.onehot_from_str_np(plate, self.global_keys["donor"]),
            "cell_type_onehot": self.onehot_from_str_np(cell_line, self.global_keys["cell_type"]),
            "pert_onehot": self.onehot_from_str_np(drugname_drugconc, self.global_keys["cytokine"]),
            "control_onehot": self.onehot_from_str_np(self.control_pert, self.global_keys["cytokine"]),
            "batch_idx": self.idx_from_str_np(plate, self.global_keys["donor"]),
            "cell_idx": self.idx_from_str_np(cell_line, self.global_keys["cell_type"]),
            "pert_idx": self.idx_from_str_np(drugname_drugconc, self.global_keys["cytokine"]),
    ...
    def onehot_from_str_np(self, s: str, vocab: list[str]) -> np.ndarray:
        i = vocab.index(s)
        x = np.zeros(len(vocab), dtype=np.int8)
        x[i] = 1
        return x
