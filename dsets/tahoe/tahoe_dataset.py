...
class Tahoe_Dataset(Dataset):
    def __init__(
                ...
                fingerprint_map: dict | None = None,
                ):
        self.conf = conf
        self.mode = mode
    ...
    def __getitem__(self, idx):
        ...
        if self.fingerprint_map is not None:
            fp = self.fingerprint_map.get(drugname_drugconc)
            if fp is None:
                fp = np.zeros(next(iter(self.fingerprint_map.values())).shape, dtype=np.float32)
            sample["pert_fingerprint"] = fp.astype(np.float32)
