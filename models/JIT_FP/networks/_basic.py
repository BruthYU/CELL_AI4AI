...
class CellMapEncoder(nn.Module):
    def __init__(self, conf: dict):
        ...
        # Replace token-wise MLP with a Transformer encoder.
        # NOTE: If d_model is not divisible by num_heads, we pad the last dim
        # (often +1) to the nearest multiple for attention, then project back.
        # Align with PRIOR: default to MLP unless explicitly set to transformer
        gene_encoder_type = self.conf.get("gene_encoder_type", "mlp")
        
        if gene_encoder_type == "transformer":
            ...
            raise ValueError(f"Unknown gene_encoder_type: {gene_encoder_type}")
        ...
        # Cellmap pooling type:
        # - "basic" (default): BasicAttentionPooling
        # - "set": SetAttentionPooling
        pooling_type = (
...
class CellMapDecoder(nn.Module):
    def __init__(self, conf: dict):
        ...
        # Align with PRIOR: decoder can be "mlp" (default) or "transformer"
...
class FiLMContinuousBlock(nn.Module):
    """Encode (fingerprint, concentration) with selectable fusion strategy.

    conf keys:
        fp_dim          - fingerprint bit-vector dimensionality (e.g. 2048)
        conc_freqs      - sinusoidal frequency count for concentration encoding (default 64)
        input_dim       - hidden dim after projection (e.g. 256)
        dims            - MLP dims applied after fusion (e.g. [256, 256])
        fp_mode         - "film" (default) or "concat"
                          film:   dose sinusoidal -> gamma/beta modulate projected fp
                          concat: raw dose scalar appended to fp, [fp_dim+1] -> MLP
        dropout_rate, act_fn, ...  - forwarded to MLPBlock
    """
    def __init__(self, conf: dict):
        super().__init__()
        self.dims = conf["dims"]
        hidden = conf["input_dim"]
        fp_dim = conf["fp_dim"]
        self.fp_mode = conf.get("fp_mode", "film")
        conc_freqs = conf.get("conc_freqs", 64)

        if self.fp_mode == "film":
            self.fp_proj = nn.Linear(fp_dim, hidden)
            self.fp_norm = nn.LayerNorm(hidden)
            self.conc_freqs = conc_freqs
            self.conc_mlp = nn.Sequential(
                nn.Linear(conc_freqs, hidden),
                nn.SiLU(),
                nn.Linear(hidden, hidden * 2),
            )
            self.mlp = MLPBlock(conf)
        elif self.fp_mode == "concat":
            concat_conf = copy.deepcopy(conf)
            concat_conf["input_dim"] = fp_dim + 1
            self.mlp = MLPBlock(concat_conf)
        else:
            raise ValueError(f"Unknown fp_mode: {self.fp_mode}, expected 'film' or 'concat'")

    def _encode_conc(self, c):
        half = self.conc_freqs // 2
        freqs = torch.exp(
            -math.log(1e4) * torch.arange(half, device=c.device, dtype=torch.float32) / half
        )
        if c.dim() > 0 and c.shape[-1] != 1:
            c = c.unsqueeze(-1)
        args = c * freqs
    ...
    def forward(self, x):
        ...
        if self.fp_mode == "film":
            h = self.fp_proj(fp_part.float())
            h = self.fp_norm(h)
            conc_emb = self._encode_conc(conc_part.float())
            gamma_beta = self.conc_mlp(conc_emb)
...
def get_customized_layer(layer_info, output_dim=None, dropout_rate=None):
    ...
    if layer_type == "embedding":
        ...
        last_dim = lay.dims[-1]
    elif layer_type == "film_continuous":
        lay = FiLMContinuousBlock(layer_info_conf)
        last_dim = lay.dims[-1]
    elif layer_type == "mlp":
