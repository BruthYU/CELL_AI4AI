...
from models.JIT_LLM_REPLOGLE_V3_STATEALIGN.flow.interpolant import Interpolant
# Velocity Field
from models.JIT_LLM_REPLOGLE_V3_STATEALIGN.networks._nemo_vf import ConditionalVelocityField
import models.JIT_LLM_REPLOGLE_V3_STATEALIGN.networks._basic as nn_basic
...
class JIT_LLM_REPLOGLE_V3_STATEALIGN_Nemo_Model(ModelInterfaceBase[MegatronModelType, MegatronLossType]):
    ...
    def configure_model(self):
        ...
        freeze_dit_until_layer = int(self.model_conf.get("freeze_dit_until_layer", 0) or 0)
        if freeze_dit_until_layer > 0 and hasattr(self.module, "dit_blocks"):
            for layer_idx, block in enumerate(self.module.dit_blocks):
                if layer_idx < freeze_dit_until_layer:
                    for param in block.parameters():
                        param.requires_grad = False
            print(f"[replogle] froze DiT layers [0, {freeze_dit_until_layer})")
    ...
    def load_from_megatron_ckpt(self, ckpt_dir: str) -> None:
        ...
        dist_env_keys = ("MASTER_ADDR", "MASTER_PORT", "RANK", "WORLD_SIZE")
        old_dist_env = {key: os.environ.get(key) for key in dist_env_keys}
        created_dist_group = False
        if not dist.is_initialized():
            os.environ["MASTER_ADDR"] = "localhost"
            os.environ["MASTER_PORT"] = str(self.find_available_port())
            os.environ["RANK"] = "0"
            os.environ["WORLD_SIZE"] = "1"
            dist.init_process_group(backend="gloo", init_method="env://", world_size=1, rank=0)
            created_dist_group = True
        try:
            full_state_dict = load_plain_tensors(ckpt_dir)
        finally:
            if created_dist_group:
                dist.destroy_process_group()
                for key, value in old_dist_env.items():
                    if value is None:
        ...
        strict = bool(self.model_conf.get("megatron_ckpt_strict", True))
        result = self.load_state_dict(model_state_dict, strict=strict)
        if strict:
            print(f"✅ Megatron checkpoint loaded successfully: {ckpt_dir}")
        else:
            print(
                "✅ Megatron checkpoint loaded with strict=False: "
                f"{ckpt_dir}; missing={len(result.missing_keys)} unexpected={len(result.unexpected_keys)}"
            )
    ...
    def validation_step(self, batch, batch_idx=None):
        ...
        print(log_info)
        self._persist_metric_log(log_info, tag="val")
        self._val_steps_in_epoch += 1
        self.log_dict(log_info, on_step=False, on_epoch=True, prog_bar=False, sync_dist=True)
        return forward_out
    ...
    def on_train_epoch_end(self):
        ...
        self._persist_metric_log(forced_log_info, tag="val_forced")
    ...
    def _persist_metric_log(self, log_info: Dict[str, Tensor], tag: str):
        ...
        out_files = [os.path.join(out_dir, f"val_metrics_forced_rank{global_rank}.jsonl")]
        if global_rank == 0:
            out_files.append(os.path.join(out_dir, "val_metrics_forced.jsonl"))
    ...
    def _build_cond_batch(self, batch, batch_size, seq_len, *, t=None):
        cond_batch = {}
        if t is not None:
            cond_batch["t"] = t
        ...
        support_keys = (
            "support_delta_other_cells",
            "support_delta_same_cell",
            "support_other_mask",
            "support_same_mask",
            "support_count",
        )
    ...
    def loss_fn(self, batch):
        ...
        cond_batch = self._build_cond_batch(batch, batch_size, seq_len, t=t)
        ...
        state_align_loss, state_align_log = self.state_align_loss(
            src_batch,
            tgt_batch,
            x1_pred,
        ...
        final_loss = (
            tgt_loss
            + self.model_conf["src_loss_weight"] * src_loss
            + flow_loss
            + state_align_loss
        )
    ...
    def eval_fn(self, batch, test_mode=False):
        ...
        cond_batch = self._build_cond_batch(batch, batch_size, seq_len)
        ...
        tgt_pred, _ = self._apply_gene_delta_head(tgt_pred, cond_batch)
        tgt_pred = self._eval_pred_transform(tgt_pred)
    ...
    def state_align_loss(self, src_batch, tgt_batch, x1_pred, cond_batch=None):
        weight = float(self.model_conf.get("state_align_loss_weight", 0.0) or 0.0)
        if weight <= 0.0:
            zero = x1_pred.new_zeros(())
            return zero, {
                "state_align_loss": 0.0,
                "state_delta_pcc_loss": 0.0,
                "state_delta_mse": 0.0,
                "state_bulk_mse": 0.0,
                "state_residual_delta_pcc_loss": 0.0,
                "state_residual_delta_mse": 0.0,
                "state_common_delta_mse": 0.0,
                "state_delta_norm_mse": 0.0,
                "state_delta_abs_mse": 0.0,
                "state_delta_smooth_l1": 0.0,
                "state_excess_delta_l1": 0.0,
                "state_delta_pcc": 0.0,
                "state_residual_delta_pcc": 0.0,
                "state_pred_delta_norm": 0.0,
                "state_target_delta_norm": 0.0,
                "state_gene_delta_abs": 0.0,
                "state_gene_delta_norm": 0.0,
                "support_raw_delta_pcc": 0.0,
                "support_residual_delta_pcc": 0.0,
                "support_residual_delta_pcc_loss": 0.0,
                "support_residual_delta_mse": 0.0,
                "support_delta_gate_mean": 0.0,
                "support_delta_mask_mean": 0.0,
            }

        pred_batch = self.module.cellmap_decoder(x1_pred)
        pred_batch, gene_delta_log = self._apply_gene_delta_head(pred_batch, cond_batch)
        pred_for_loss = self._state_align_transform(pred_batch)

        src_mean = src_batch.float().mean(dim=1)
        tgt_mean = tgt_batch.float().mean(dim=1)
        pred_mean = pred_for_loss.float().mean(dim=1)

        target_delta = tgt_mean - src_mean
        pred_delta = pred_mean - src_mean

        pred_centered = pred_delta - pred_delta.mean(dim=-1, keepdim=True)
        target_centered = target_delta - target_delta.mean(dim=-1, keepdim=True)
        delta_pcc = F.cosine_similarity(pred_centered, target_centered, dim=-1, eps=1e-8).mean()
        delta_pcc_loss = 1.0 - delta_pcc
        delta_mse = F.mse_loss(pred_delta, target_delta)
        bulk_mse = F.mse_loss(pred_mean, tgt_mean)

        pred_common_delta = pred_delta.mean(dim=0, keepdim=True)
        target_common_delta = target_delta.mean(dim=0, keepdim=True)
        pred_residual_delta = pred_delta - pred_common_delta
        target_residual_delta = target_delta - target_common_delta
        pred_residual_centered = pred_residual_delta - pred_residual_delta.mean(dim=-1, keepdim=True)
        target_residual_centered = target_residual_delta - target_residual_delta.mean(dim=-1, keepdim=True)
        residual_delta_pcc = F.cosine_similarity(
            pred_residual_centered,
            target_residual_centered,
            dim=-1,
            eps=1e-8,
        ).mean()
        residual_delta_pcc_loss = 1.0 - residual_delta_pcc
        residual_delta_mse = F.mse_loss(pred_residual_delta, target_residual_delta)
        common_delta_mse = F.mse_loss(pred_common_delta, target_common_delta)
        pred_delta_norm_per = pred_centered.norm(dim=-1)
        target_delta_norm_per = target_centered.norm(dim=-1)
        delta_norm_mse = F.mse_loss(torch.log1p(pred_delta_norm_per), torch.log1p(target_delta_norm_per))
        delta_abs_mse = F.mse_loss(torch.log1p(pred_delta.abs()), torch.log1p(target_delta.abs()))
        delta_smooth_l1 = F.smooth_l1_loss(pred_delta, target_delta)
        excess_delta_l1 = F.relu(pred_delta.abs() - target_delta.abs()).mean()
        pred_delta_norm = pred_delta_norm_per.mean()
        target_delta_norm = target_delta_norm_per.mean()
        support_log = self._support_residual_loss(pred_delta, target_delta, cond_batch)

        pcc_weight = float(self.model_conf.get("state_align_delta_pcc_weight", 1.0))
        delta_mse_weight = float(self.model_conf.get("state_align_delta_mse_weight", 0.0))
        bulk_mse_weight = float(self.model_conf.get("state_align_bulk_mse_weight", 0.0))
        residual_pcc_weight = float(self.model_conf.get("state_align_residual_delta_pcc_weight", 0.0))
        residual_mse_weight = float(self.model_conf.get("state_align_residual_delta_mse_weight", 0.0))
        common_mse_weight = float(self.model_conf.get("state_align_common_delta_mse_weight", 0.0))
        delta_norm_mse_weight = float(self.model_conf.get("state_align_delta_norm_mse_weight", 0.0))
        delta_abs_mse_weight = float(self.model_conf.get("state_align_delta_abs_mse_weight", 0.0))
        delta_smooth_l1_weight = float(self.model_conf.get("state_align_delta_smooth_l1_weight", 0.0))
        excess_delta_l1_weight = float(self.model_conf.get("state_align_excess_delta_l1_weight", 0.0))
        support_residual_pcc_weight = float(self.model_conf.get("support_residual_delta_pcc_weight", 0.0))
        support_residual_mse_weight = float(self.model_conf.get("support_residual_delta_mse_weight", 0.0))
        support_gate_l1_weight = float(self.model_conf.get("support_delta_gate_l1_weight", 0.0))
        loss = weight * (
            pcc_weight * delta_pcc_loss
            + delta_mse_weight * delta_mse
            + bulk_mse_weight * bulk_mse
            + residual_pcc_weight * residual_delta_pcc_loss
            + residual_mse_weight * residual_delta_mse
            + common_mse_weight * common_delta_mse
            + delta_norm_mse_weight * delta_norm_mse
            + delta_abs_mse_weight * delta_abs_mse
            + delta_smooth_l1_weight * delta_smooth_l1
            + excess_delta_l1_weight * excess_delta_l1
            + support_residual_pcc_weight * support_log["support_residual_delta_pcc_loss_tensor"]
            + support_residual_mse_weight * support_log["support_residual_delta_mse_tensor"]
            + support_gate_l1_weight * support_log["support_delta_gate_l1_tensor"]
        )
        return loss, {
            "state_align_loss": float(loss.detach().cpu()),
            "state_delta_pcc_loss": float(delta_pcc_loss.detach().cpu()),
            "state_delta_mse": float(delta_mse.detach().cpu()),
            "state_bulk_mse": float(bulk_mse.detach().cpu()),
            "state_residual_delta_pcc_loss": float(residual_delta_pcc_loss.detach().cpu()),
            "state_residual_delta_mse": float(residual_delta_mse.detach().cpu()),
            "state_common_delta_mse": float(common_delta_mse.detach().cpu()),
            "state_delta_norm_mse": float(delta_norm_mse.detach().cpu()),
            "state_delta_abs_mse": float(delta_abs_mse.detach().cpu()),
            "state_delta_smooth_l1": float(delta_smooth_l1.detach().cpu()),
            "state_excess_delta_l1": float(excess_delta_l1.detach().cpu()),
            "state_delta_pcc": float(delta_pcc.detach().cpu()),
            "state_residual_delta_pcc": float(residual_delta_pcc.detach().cpu()),
            "state_pred_delta_norm": float(pred_delta_norm.detach().cpu()),
            "state_target_delta_norm": float(target_delta_norm.detach().cpu()),
            "support_raw_delta_pcc": float(support_log["support_raw_delta_pcc"].detach().cpu()),
            "support_residual_delta_pcc": float(support_log["support_residual_delta_pcc"].detach().cpu()),
            "support_residual_delta_pcc_loss": float(support_log["support_residual_delta_pcc_loss_tensor"].detach().cpu()),
            "support_residual_delta_mse": float(support_log["support_residual_delta_mse_tensor"].detach().cpu()),
            "support_delta_gate_mean": float(support_log["support_delta_gate_mean"].detach().cpu()),
            "support_delta_mask_mean": float(support_log["support_delta_mask_mean"].detach().cpu()),
            **gene_delta_log,
        }

    def _support_residual_loss(self, pred_delta, target_delta, cond_batch):
        zero = pred_delta.new_zeros(())
        out = {
            "support_raw_delta_pcc": zero,
            "support_residual_delta_pcc": zero,
            "support_residual_delta_pcc_loss_tensor": zero,
            "support_residual_delta_mse_tensor": zero,
            "support_delta_gate_l1_tensor": zero,
            "support_delta_gate_mean": zero,
            "support_delta_mask_mean": zero,
        }
        if cond_batch is None:
            return out
        support_fn = getattr(self.module, "support_delta_components", None)
        if support_fn is None:
            return out
        support = support_fn(cond_batch, apply_dropout=False)
        if support is None:
            return out

        source_delta = support["source_delta"].to(device=pred_delta.device, dtype=pred_delta.dtype)
        source_mask = support["source_mask"].to(device=pred_delta.device, dtype=pred_delta.dtype)
        if source_mask.dim() == 1:
            source_mask = source_mask.unsqueeze(-1)
        sample_mask = source_mask.squeeze(-1)
        mask_sum = sample_mask.sum().clamp_min(1.0)
        out["support_delta_mask_mean"] = sample_mask.mean()
        out["support_delta_gate_mean"] = (support["gate"].to(pred_delta.dtype).squeeze(-1) * sample_mask).sum() / mask_sum
        out["support_delta_gate_l1_tensor"] = out["support_delta_gate_mean"]

        source_centered = source_delta - source_delta.mean(dim=-1, keepdim=True)
        target_centered = target_delta - target_delta.mean(dim=-1, keepdim=True)
        raw_pcc_per = F.cosine_similarity(source_centered, target_centered, dim=-1, eps=1e-8)
        out["support_raw_delta_pcc"] = (raw_pcc_per * sample_mask).sum() / mask_sum

        pred_residual = (pred_delta - source_delta) * source_mask
        target_residual = (target_delta - source_delta) * source_mask
        pred_centered = pred_residual - pred_residual.mean(dim=-1, keepdim=True)
        target_centered = target_residual - target_residual.mean(dim=-1, keepdim=True)
        residual_pcc_per = F.cosine_similarity(pred_centered, target_centered, dim=-1, eps=1e-8)
        residual_pcc = (residual_pcc_per * sample_mask).sum() / mask_sum
        out["support_residual_delta_pcc"] = residual_pcc
        out["support_residual_delta_pcc_loss_tensor"] = 1.0 - residual_pcc
        denom = (source_mask.sum() * pred_delta.shape[-1]).clamp_min(1.0)
        out["support_residual_delta_mse_tensor"] = ((pred_residual - target_residual) ** 2).sum() / denom
        return out

    def _apply_gene_delta_head(self, pred_batch, cond_batch):
        zero_log = {
            "state_gene_delta_abs": 0.0,
            "state_gene_delta_norm": 0.0,
        }
        if cond_batch is None:
            return pred_batch, zero_log
        predict_gene_delta = getattr(self.module, "predict_gene_delta", None)
        if predict_gene_delta is None:
            return pred_batch, zero_log
        gene_delta = predict_gene_delta(cond_batch)
        if gene_delta is None:
            return pred_batch, zero_log
        scale = float(self.model_conf.get("gene_delta_head_scale", 1.0))
        pred_batch = pred_batch + scale * gene_delta.unsqueeze(1)
        return pred_batch, {
            "state_gene_delta_abs": float(gene_delta.detach().abs().mean().cpu()),
            "state_gene_delta_norm": float(gene_delta.detach().norm(dim=-1).mean().cpu()),
        }

    def _state_align_transform(self, pred_batch):
        transform = str(self.model_conf.get("state_align_pred_transform", "none")).lower()
        return self._apply_pred_transform(pred_batch, transform, "state_align_pred_transform")

    def _eval_pred_transform(self, pred_batch):
        transform = str(self.model_conf.get("eval_pred_transform", "none")).lower()
        return self._apply_pred_transform(pred_batch, transform, "eval_pred_transform")

    def _apply_pred_transform(self, pred_batch, transform, name):
        if transform in {"none", "", "identity"}:
            return pred_batch
        if transform in {"clip_zero", "clamp_zero", "relu"}:
            return torch.clamp(pred_batch, min=0.0)
        if transform == "softplus":
            return F.softplus(pred_batch)
        raise ValueError(f"Unknown {name}: {transform}")
