...
from models.SCALE.flow.interpolant import Interpolant
# Velocity Field
from models.SCALE.networks._nemo_vf import ConditionalVelocityField
...
class SCALE_Nemo_Model(ModelInterfaceBase[MegatronModelType, MegatronLossType]):
    ...
    def loss_fn(self, batch):
        ...
        tgt_decoded_map = self.module.tgt_cellmap_decoder(tgt_encoded_map)
        ...
        src_decoded_map = self.module.tgt_cellmap_decoder(src_encoded_map)
        ...
        for k in cond_keys:
            # cond_batch[k] = batch[k].reshape(batch_size, seq_len, -1) # [B, N, one_hot_dim]
            cond_batch[k] = batch[k].unsqueeze(1).expand(-1, seq_len, -1)
        
        
        pred_vf = self.module(x_t, cond_batch)
        flow_loss = self.metric_utils.flow_mse(pred_vf, v_t)

        final_loss = tgt_vae_mse + tgt_vae_mmd + src_vae_mse + src_vae_mmd + flow_loss
        ...
        log_info = {"src_vae_mse": src_vae_mse.item(), "src_vae_mmd": src_vae_mmd.item(), "flow_loss": flow_loss.item(), "final_loss": final_loss.item()}
    ...
    def eval_fn(self, batch, test_mode=False):
        ...
        # Gather and Reshape Batch
        device = batch["ctrl_cell_emb"].device
        src_batch = batch["ctrl_cell_emb"].reshape(batch_size, seq_len, cell_dim) # [B, N, D]
        tgt_batch = batch["pert_cell_emb"].reshape(batch_size, seq_len, cell_dim) # [B, N, D]

        
        # no flow prediction
        src_encoded_map = self.module.tgt_cellmap_encoder(src_batch)
        tgt_decoded_map = self.module.tgt_cellmap_decoder(src_encoded_map)
        ...
        tgt_pred = self.module.tgt_cellmap_decoder(x_t)
