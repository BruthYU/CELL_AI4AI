#!/usr/bin/env python3
"""Build full-cell Replogle h5ads with train-only delta memory."""
...
def read_full_csr(handle: h5py.File) -> sp.csr_matrix:
    x = handle["X"]
...
def compact_memory_delta(
    ...
    *,
    ...
    with h5py.File(memory_pred_path, "r") as handle:
        ...
        matrix = read_full_csr(handle)

    control_mask = genes == control_pert
    if not control_mask.any():
        raise ValueError(f"{memory_pred_path} has no {control_pert!r} control row")
    control_mean = np.asarray(matrix[control_mask].mean(axis=0)).ravel().astype(np.float32)

    deltas: dict[str, np.ndarray] = {}
    zero_delta_genes: set[str] = set()
    counts: dict[str, int] = {}
    for gene in np.unique(genes[~control_mask]):
        mask = genes == gene
        delta = np.asarray(matrix[mask].mean(axis=0)).ravel().astype(np.float32) - control_mean
        gene_name = str(gene)
        deltas[gene_name] = delta
        counts[gene_name] = int(mask.sum())
        if float(np.linalg.norm(delta)) <= float(zero_atol):
            zero_delta_genes.add(gene_name)
    return deltas, zero_delta_genes, counts


def load_expected_primary_missing_counts(summary_path: Path | None) -> dict[str, int]:
    if summary_path is None or not summary_path.exists():
        return {}
    data = json.loads(summary_path.read_text())
    out: dict[str, int] = {}
    for row in data.get("cell_lines", []):
        cell_line = row.get("cell_line")
        count = row.get("num_missing_test_genes")
        if cell_line is not None and count is not None:
            out[str(cell_line).lower()] = int(count)
    return out
...
def memory_for_gene(
    ...
    *,
    n_vars: int,
    primary_delta: dict[str, np.ndarray],
    primary_zero_delta_genes: set[str],
    fallback_delta: dict[str, np.ndarray],
) -> tuple[np.ndarray, str]:
    if gene in primary_delta and gene not in primary_zero_delta_genes:
        return primary_delta[gene], "D_same_gene_other_cellline"
    if gene in fallback_delta:
        return fallback_delta[gene], "D_same_cell_other_pert"
    return np.zeros(n_vars, dtype=np.float32), "D_global_train_delta_zero"
...
def build_one_cell_line(
    *,
    ...
    clip_min: float | None,
    zero_atol: float,
    expected_primary_missing_count: int | None,
) -> dict[str, object]:
    primary_delta, primary_zero_delta_genes, primary_counts = compact_memory_delta(
        primary_memory_path,
        control_pert=control_pert,
        zero_atol=zero_atol,
    )
    fallback_delta, fallback_zero_delta_genes, fallback_counts = compact_memory_delta(
        fallback_memory_path,
        control_pert=control_pert,
        zero_atol=zero_atol,
    )

    if expected_primary_missing_count is not None and len(primary_zero_delta_genes) != expected_primary_missing_count:
        raise ValueError(
            f"{cell_line}: primary zero-delta genes={len(primary_zero_delta_genes)} "
            f"!= expected num_missing_test_genes={expected_primary_missing_count}"
        )
    ...
    mode_gene_counts = {
        "D_same_gene_other_cellline": 0,
        "D_same_cell_other_pert": 0,
        "D_global_train_delta_zero": 0,
    }
    mode_row_counts = dict(mode_gene_counts)

    with h5py.File(raw_pred_path, "r") as src, h5py.File(out_pred_path, "w") as dst:
        n_obs, n_vars = h5ad_shape(src)
        genes_obs = obs_column(src, "gene").astype(str)
        runs = contiguous_runs(genes_obs)
        raw_x = src["X"]
        ...
        while run_idx < len(runs):
            ...
            pred_cells = read_csr_rows(raw_x, target_start, target_end, n_vars)
            ctrl_cells = read_csr_rows(raw_x, control_start, control_end, n_vars)
            memory_delta, mode = memory_for_gene(
                gene,
                n_vars=n_vars,
                primary_delta=primary_delta,
                primary_zero_delta_genes=primary_zero_delta_genes,
                fallback_delta=fallback_delta,
            )
            n_target_rows = int(target_end - target_start)
            mode_gene_counts[mode] += 1
            mode_row_counts[mode] += n_target_rows
    ...
    return {
        "cell_line": cell_line,
        "canonical_method": "model_plus_delta_memory",
        "delta_memory_rule": "prefer D_same_gene_other_cellline; fallback to D_same_cell_other_pert when D_same_gene_other_cellline is unavailable; otherwise zero D_global_train_delta",
        ...
        "primary_branch": "D_same_gene_other_cellline",
        "fallback_branch": "D_same_cell_other_pert",
        "zero_fallback_branch": "D_global_train_delta_zero",
        "primary_unavailable_detection": f"compact D_same_gene_other_cellline delta L2 norm <= {zero_atol}",
        ...
        "primary_memory_path": str(primary_memory_path),
        ...
        "mode_gene_counts": mode_gene_counts,
        "mode_row_counts": mode_row_counts,
        "primary_num_genes": int(len(primary_delta)),
        "primary_zero_delta_genes": int(len(primary_zero_delta_genes)),
        "primary_zero_delta_expected": expected_primary_missing_count,
        "fallback_num_genes": int(len(fallback_delta)),
        "fallback_zero_delta_genes": int(len(fallback_zero_delta_genes)),
        "primary_rows_by_gene_min": int(min(primary_counts.values())) if primary_counts else 0,
        "primary_rows_by_gene_max": int(max(primary_counts.values())) if primary_counts else 0,
        "fallback_rows_by_gene_min": int(min(fallback_counts.values())) if fallback_counts else 0,
        "fallback_rows_by_gene_max": int(max(fallback_counts.values())) if fallback_counts else 0,
        "nnz": int(nnz_so_far),
    }


def default_primary_summary(primary_memory_dir: Path) -> Path:
    return primary_memory_dir.parent / "train_delta_crosscell_baseline_summary.json"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional summary with num_missing_test_genes. Defaults to primary-memory-dir parent summary if present.",
    )
    ...
    primary_summary = args.primary_summary_json
    if primary_summary is None:
        candidate = default_primary_summary(args.primary_memory_dir)
        primary_summary = candidate if candidate.exists() else None
    expected_primary_missing = load_expected_primary_missing_counts(primary_summary)
    ...
    for cell_line in [str(x).lower() for x in args.cell_lines]:
        ...
        for path in (raw_pred_path, raw_real_path, primary_memory_path, fallback_memory_path):
            if not path.exists():
        ...
        print(
            ...
            "with fallback D_same_cell_other_pert",
            flush=True,
        )
        summary = build_one_cell_line(
            ...
            primary_memory_path=primary_memory_path,
            fallback_memory_path=fallback_memory_path,
            ...
            expected_primary_missing_count=expected_primary_missing.get(cell_line),
        )
        summaries.append(summary)
        print(
            f"wrote {out_pred_path} rows={summary['rows']} "
            f"mode_gene_counts={summary['mode_gene_counts']} nnz={summary['nnz']}",
            flush=True,
        )
    ...
    manifest = {
        ...
        "model_weight": float(args.model_weight),
        "memory_weight": float(args.memory_weight),
        "primary_memory_dir": str(args.primary_memory_dir),
        "fallback_memory_dir": str(args.fallback_memory_dir),
        "primary_summary_json": str(primary_summary) if primary_summary is not None else None,
        "cell_lines": summaries,
    }
    manifest_path = args.out_dir / "model_plus_delta_memory_manifest.json"
